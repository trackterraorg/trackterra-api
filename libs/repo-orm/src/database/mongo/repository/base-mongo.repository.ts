...
import { cleanEmptyProperties, NotFoundError } from '@trackterra/common';
...
export class BaseMongoRepository<DOC, DTO = DOC> {
  ...
  async findCursor(
    ...
    if (orderField === 'id') {
      const [count, docs] = await Promise.all([
        await collection.find(cleanConditions).count(),
  ...
  async create(document: Partial<DTO> | DTO): Promise<DOC> {
    ...
    const cleanDoc = { ...eventResult, ...cleanEmptyProperties({}) };
